...
def read_imgs(file_path,freq):
    images_path_lines = csv.reader(open(file_path + 'data.csv','r'))
    imgs = []
    imgs_timestep = []
    index = -1
    for image_path in images_path_lines:
        ...
        #print( 'image_path full:',image_path_full)
        imgs.append(image_path_full)
    return imgs,imgs_timestep

### jiayao save desc for bvt validation
def save_desc(model_dir, data_path, num_include=5,
                    title='Precision-Recall Curve', checkpoint=None,
                    netvlad_feat=None, include_calc=False):
    ...
    print('total mem files: ',len(mem_files),' total live files ',len(live_files))
    ...
    #        t_calc.append(time()-t0)
            ...
            print('total database:',len(live_database),'time:',t_calc[-1],' ms')
            pickle.dump(live_database,open('./res/query_img_desc_calc.txt','wb'))
    #         kp, kp_d = kp_descriptor(c5)
    #
    #         if netvlad_feat is not None:
    #             sim_nv = np.sum(q_netvlad[i:i+1,:] * db_netvlad, axis=-1)
